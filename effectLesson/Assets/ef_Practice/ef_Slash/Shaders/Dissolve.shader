Shader "ShaderPra/Dissolve" //シェーダー名と、マテリアル上でのシェーダー選択時の階層(通称: シェーダー宣言)
{
    Properties //マテリアルのインスペクターに表示する設定。
    {            
        _MainTex ("NoiseTexture", 2D) = "white" {} //ディゾルブ表現用のノイズテクスチャを指定。お好みでどうぞ。
        _ScrollFadeOut("_ScrollFadeOut", 2D) = "White" {} //UVスクロール時のフェードアウトを白黒グラデ画像で指定。お好みで。　
        _FireAlpha("FireAlpha", Range(0,10)) = 0 //炎全体の透明度(兼、Emissionの強さ)。下げて消える。上げて強く光る。
    }
        //この作例は入門編なので、分かりやすく画像を使っていますが、
        //負荷や手間の面で不都合ないなら、コードでノイズやグラデを生成でも良いと思います。
        //ちなみに、ShaderGraphで作る場合はノイズやグラデ生成のノードがあります。
    ...
    SubShader //シェーダーの実行単位。シェーダー宣言内に複数記述可能。Unityが各SubShaderの実行条件からどれを実行するか判断。
    {
        ...
        Pass //実行される処理内容
        {
            ...
            {
                //この作例では、4つの入力用の変数があり、以下の様に『型 変数名 : セマンティクス』の順に記述します。
                //重要なのが『:セマンティクス』です。構造体内の全変数名にその役割を対応づけ(通称: バインド)して、
                //その変数の意味を、システムに対して明示する役割があります。
                //そして、セマンティクスには、入力・出力があり、それぞれ使用できるものが決まっています。
                ...
                //Shurikenとシェーダーの合わせ技の基本設定として、
                //まず、上記のappdata構造体のセマンティクスと、
                //Shuriken側のRendrer>Custom Vertex Streamのセマンティクスを、合致させます。
                //合致していないと、エラーが出るはず…(汗)
                
                //一点、作例を分かりやすくするため、customDataにTEXCOORD1.xyzwが切りよく割り当てられるよう、
                //Shuriken側のRendrer>Custom Vertex Streamで、「UV2(TEXCOORD0.zw)」を余分に追加しています。
                //シェーダー側のセマンティクスは自分で記述できますが、Shuriken側は、プルダウン選択時に自動割当されるので
                //その辺りが不便ですよね…。(何かその辺りの良い解決方法ご存知の方いたらご教示願いたいです…)
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                return noise * i.color.a * _FireAlpha;//最後にnoiseに頂点カラーのアルファと炎全体の透明度(兼、Emissionの強さ)を乗算。
            }
            ENDCG
        }
    }
